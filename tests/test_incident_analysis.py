...
async def test_openai_adapter_requests_strict_non_stored_structured_output() -> None:
    ...
    output = {
        ...
        "conclusion": {
            "statement": "The available evidence indicates provider processing degradation.",
    ...
    assert result.conclusion.verification == "estimated"
...
async def test_openai_adapter_accepts_supplied_external_signal_reference() -> None:
    ...
    output = {
        ...
        "conclusion": {
            "statement": "The supplied status signal corroborates the provider degradation.",
            "evidence_refs": ["external:status_001", "snapshot"],
        },
        "response_code_explanations": [],
        "recommended_actions": [
...
@pytest.mark.asyncio
async def test_openai_adapter_explains_only_unknown_codes_and_verifies_counts() -> None:
    base = evidence()
    ...
    output = {
        ...
        "causes": [
            {
                "category": "technical",
                "title": "Undocumented upstream rejection",
                "why": "The provider returned APX_771 with HTTP 502.",
                "evidence_refs": ["event:evt-unknown", "snapshot"],
            }
        ],
        "conclusion": {
            "statement": "The evidence supports an upstream AtlasPay processing failure.",
            "evidence_refs": ["event:evt-unknown", "snapshot"],
        },
        "response_code_explanations": [
            {
                "response_code": "APX_771",
                "name": "Uncatalogued upstream failure",
                "description": "The sampled HTTP 502 response suggests an upstream processing failure; the exact provider meaning remains unverified.",
                "evidence_refs": ["event:evt-unknown"],
            }
        ],
        "recommended_actions": [
            {
                "priority": 1,
                "title": "Confirm with AtlasPay",
                "rationale": "Validate the undocumented code before mitigation.",
            }
        ],
        "confidence": 0.72,
    }
    ...
    assert model_input["unresolved_response_codes"] == ["APX_771"]
    assert model_input["internal_response_catalog"][0]["response_code"] == "APPROVED"
    assert schema["$defs"]["_ProposedResponseCodeInsight"]["properties"][  # type: ignore[index]
        "response_code"
    ]["enum"] == ["APX_771"]
    assert [item.source for item in result.response_code_insights] == [
        ResponseInsightSource.CATALOG,
        ResponseInsightSource.OPENAI,
    ]
    assert result.conclusion is not None
    assert result.conclusion.verification == "verified"
    assert result.conclusion.affected_requests == 20
    assert result.conclusion.total_requests == 100
    assert result.conclusion.payment_methods[0].affected_requests == 10
    await client.aclose()
