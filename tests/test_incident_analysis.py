...
@pytest.mark.asyncio
async def test_mock_analyzer_produces_operator_advisory() -> None:
    result = await MockIncidentAnalyzer().analyze(evidence())

    assert result.generated_by is AnalysisProvider.MOCK
    assert result.recommended_actions
    assert all(not action.safe_to_automate for action in result.recommended_actions)
